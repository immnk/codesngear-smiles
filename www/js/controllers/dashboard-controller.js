...
DashboardController.inject = ['$scope', '$http', '$state', '$ionicPopup', "$ionicLoading", 'ActivityFactory'];
    ...
    $scope.data.initialize = function() {
        ActivityFactory.getActivitiesList()
            .then(function(response) {
                ...
                for (i = 0; i < $scope.data.activities.length; i++) {
                    var activity = $scope.data.activities[i];
                    console.log(activity);
                    ActivityFactory.getWishListTotalValue(activity.wishlistId)
                        .then(function(response) {
                            activity.total = response.total;
                            ActivityFactory.getWishlistItemsReceivedCost(activity.wishlistId)
                                .then(function(response) {
                                    activity.contributed = response.contributed;
